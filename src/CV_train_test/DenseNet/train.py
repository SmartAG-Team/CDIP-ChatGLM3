...
def main(args):
    ...
    for epoch in range(args.epochs):
        mean_loss, accuracy, learning_rate = train_one_epoch(model=model,
                                                             optimizer=optimizer,
                                                             data_loader=train_loader,
                                                             device=device,
                                                             epoch=epoch)

        val_accuracy, val_recall, val_f1 = evaluate(model=model,
                                                    data_loader=val_loader,
                                                    device=device)

        print("[epoch {}] accuracy: {}, recall: {}, F1: {}".format(epoch, round(val_accuracy, 3),
                                                                   round(val_recall, 3), round(val_f1, 3)))

        tags = ["loss", "accuracy", "learning_rate", "recall", "F1"]
        tb_writer.add_scalar(tags[0], mean_loss, epoch)
        tb_writer.add_scalar(tags[1], accuracy, epoch)
        tb_writer.add_scalar(tags[2], learning_rate, epoch)
