...
parser.add_argument('--instruction_number', type=int, default=3, help='Number of instructions proposed per txt file')
...
with open(args.prompt3_dir, 'r', encoding='utf-8') as prompt_file:
    prompt3 = prompt_file.read()
...
instruction_file_path = f'./data/LLM/LLM_dataset/13-Crop-Instruction-Following-Dataset/{args.varieties}/{folder_name}_instruction.json'
original_output_file_path = f'./data/LLM/LLM_dataset/13-Crop-Instruction-Following-Dataset/{args.varieties}/{folder_name}_original_output.json'
output_file_path = f'./data/LLM/LLM_dataset/13-Crop-Instruction-Following-Dataset/{args.varieties}/{folder_name}_finetune_output.json'
...
def output(original_output_file_path):
    ...
    for index, data in enumerate(split_data):
        try:
            text = prompt3 + str(data) + '给定的上下文' + '/n' + txt_files_content[index] 
            # print(text)
            # 创建prompt并调用API
            completion = client.chat.completions.create(
                model=args.model,
                messages=[
                    ...
                    {'role': 'user', 'content': '请结合上下文信息针对每一个指令修改润色一下回复信息。'}
                ],
                stream=False
            )
...
if __name__ == "__main__":
    instruction_generation(folder_path) #Step1
    original_output(instruction_file_path)  #Step2
    output(original_output_file_path)  #Step3
