...
parser.add_argument('--api_key', type=str, default=os.getenv('Qwen_API_Key'), help='api key') 
parser.add_argument('--model', type=str, default='qwen2.5-vl-72b-instruct', help='model name')
...
# API 客户端配置（确保 API Key 和 base_url 正确）
client = OpenAI(
...
# 结果文件路径
output_file = "./data/CV/CV_Metric/VLM-Test-Metric/Qwen2.5-VL-72B.txt"                              
    ...
    for image_file in os.listdir(subfolder_path):
        if image_file.lower().endswith(('.png', '.jpg', '.jpeg', '.bmp', '.gif')):
            ...
            try:
                ...
                # 如果图片大于 1MB，则进行压缩
                if len(image_data) > 1 * 1024 * 1024:  # 1MB = 1*1024*1024 bytes
                    try:
            ...
            try:
                ...
                # 如果回复不在候选答案中，则尝试用 difflib 寻找最接近的答案
