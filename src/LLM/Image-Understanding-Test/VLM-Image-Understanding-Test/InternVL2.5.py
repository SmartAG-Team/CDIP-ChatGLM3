...
parser.add_argument('--api_key', type=str, default=os.getenv('InternVL_API_Key'), help='api key') 
parser.add_argument('--base_url',type=str,default=os.getenv('InternVL_API_base_url'))
parser.add_argument('--model', type=str, default='InternVL2.5-78B', help='model name')
...
# 结果文件路径
output_file = "./data/CV/CV_Metric/VLM-Test-Metric/InternVL2.5-78B.txt"                              
    ...
    for image_file in os.listdir(subfolder_path):
        if image_file.lower().endswith(('.png', '.jpg', '.jpeg', '.bmp', '.gif')):
                ...
                if len(image_data) > 1 * 1024 * 1024:
                    try:
                        ...
                        while True:
                            ...
                            if len(compressed_data) <= 1 * 1024 * 1024 or quality <= 20:
                                break
                            quality -= 5
                        image_data = compressed_data
                        mime = "image/jpeg"
                        print(f"    Compressed {image_file} to {len(image_data)/1024:.2f}KB (quality={quality})")
                    except Exception as comp_e:
                        print(f"    Error compressing {image_file}: {comp_e}")
                        # 压缩失败则根据后缀确定 MIME 类型
            ...
            try:
                ...
                # 如果回复不在候选答案中，则使用 difflib 寻找最接近的答案
