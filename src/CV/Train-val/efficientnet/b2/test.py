...
def main():
    ...
    model_weight_path = "./model/CV_Models/trained_model_1/efficientnetb2.pth"
    ...
    # 保存混淆矩阵到 CSV
    class_names = [dataset.class_indict[str(i)] for i in range(len(dataset.class_indict))]
    cm = confusion_matrix(all_targets, all_predictions)

    output_dir = './data/CV/CV_Metric'
    os.makedirs(output_dir, exist_ok=True)  # 确保文件夹存在

    cm_output_path = os.path.join(output_dir, 'CV_confusion_matrix.csv')
    save_full_cm_to_csv(cm, class_names, cm_output_path)
