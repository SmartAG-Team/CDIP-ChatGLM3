...
with open('./data/fine-tuning_dataset/grape/question.json', 'r', encoding='utf-8') as question_file:
    instructions = json.load(question_file)  

# 读取prompt文件
with open('./src/create_fine-tuning_dataset/prompt4.txt', 'r', encoding='utf-8') as prompt_file:
    prompt = prompt_file.read()

# 读取上下文资料
with open('./data/books/test/一 霜霉病.txt', 'r', encoding='utf-8') as context_file:
...
for item in instructions:
    try:
        ...
        output = item['output']
        text = instruction + '/n' + '回复：' + '/n'  + output
        ...
        completion = client.chat.completions.create(
            ...
            messages=[
                ...
                {'role': 'system', 'content': '给定的上下文'},
                {'role': 'system', 'content': context},
                {'role': 'user', 'content': '请结合上下文信息润色一下回复信息'}
            ],
            stream=False
        )
...
print("Output has been written to output_2.json")
